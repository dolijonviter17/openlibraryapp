...
const CreateNewBookScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const handleInsertNewBook = async (book: any) => {
    try {
      await db.transaction(async (tx) => {
        await tx.executeSql(
          "INSERT INTO books (full_url, name, seed_count, url, last_update) VALUES (?, ?, ?, ?, ?)",
          [
            book.full_url,
            book.name,
            parseInt(book.seed_count),
            book.url,
            book.last_update,
          ]
        );
  ...
  return (
    <View style={styles.flex}>
      <HeaderComponent title="Create New Book" onPress={handleBack} />
      <ScrollView
        style={{
      ...
      <TouchableOpacity onPress={() => handleInsertNewBook(inputBook)}>
        <View style={styles.btnSave}>
          <Text style={TEXT_STYLES.text500}>{"Save Books"}</Text>
        </View>
      </TouchableOpacity>
    </View>
  );
...
export default CreateNewBookScreen;
